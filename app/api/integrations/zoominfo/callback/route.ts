...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.redirect(new URL("/lists?zoominfo=connected", request.url));
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unable to complete ZoomInfo authorization";
    return NextResponse.redirect(new URL(`/lists?zoominfo=error&message=${encodeURIComponent(message)}`, request.url));
  }
}
