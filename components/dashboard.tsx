...
export function Dashboard({ initialDetails, initialStatus, metrics, initialAccountId, initialStage }: DashboardProps) {
  ...
  const firmographicFacts = firmographics
    ? [firmographics.industry, selected.account.revenueRange, firmographics.employeeCount ? `${firmographics.employeeCount.toLocaleString()} employees` : undefined, firmographics.hqLocation, firmographics.companyType, firmographics.ticker].filter(Boolean) as string[]
    : [];
